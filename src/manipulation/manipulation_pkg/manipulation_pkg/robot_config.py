...
DROP_JOINTS_DEG      = [-35.0, -25.0,   0.0,  75.0, -160.0, -10.0, -115.0]

# ── cartesian waypoints ────────────────────────────────────────────
# (x, y, z in metres, roll/pitch/yaw in degrees)
# grasp orientation
GRASP_RPY = dict(roll=-90.0, pitch=0.0, yaw=-180.0)
# drop orientation  
DROP_RPY  = dict(roll=-86.5, pitch=3.5, yaw=-121.0)

# (x, y, z in metres, roll/pitch/yaw in degrees)
GRASP_POSE = dict(x=0.0,   y=-0.350, z=0.160, **GRASP_RPY)
LIFT_POSE  = dict(x=0.0,   y=-0.350, z=0.350, **GRASP_RPY)
