...
// SV input: DC 0–5V (or PWM 5V, 1kHz–2kHz) → 0 to 70 RPM (rated speed)
...
void setup()
{
    ...
    digitalWrite(FR, LOW); // LOW is CW in auger perspective
    digitalWrite(EN, LOW); // enable driver (LOW = enabled)

    Serial.println("Enter RPM (0-70):");
}
...
void loop()
{
    ...
    {
        currentRPM = constrain(targetRPM, 0, RATED_RPM);
